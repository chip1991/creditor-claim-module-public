...
- [x] Task 10: 后端组织管理改造为独立增删改
  - [x] 新增组织创建、编辑、删除接口，替代页面主同步流
  - [x] 增加删除前校验：存在下级组织或关联用户时禁止删除，并返回中文提示
  - [x] 审计覆盖组织新增、编辑、删除

- [x] Task 11: 后端角色与用户主数据改造为独立增删改
  - [x] 新增角色创建、编辑、删除接口，并保留角色详情三件套配置能力
  - [x] 新增用户创建、编辑、删除接口，并保持用户角色分配与启停能力
  - [x] 增加删除前校验：角色已分配给用户时禁止删除
  - [x] 审计覆盖角色新增、编辑、删除与用户新增、编辑、删除

- [x] Task 12: 前端三个页面改为 CRUD 交互
  - [x] 组织管理页移除“同步组织架构树”，新增“新建组织/编辑/删除”入口与表单交互
  - [x] 角色管理页移除“同步 IAM 岗位”，新增“新建角色/编辑/删除”入口，并保持进入角色详情页配置权限
  - [x] 用户管理页移除“全量同步 IAM 用户”，新增“新建用户/编辑/删除”入口，并保留分配角色与启停
  - [x] 所有系统级文案保持中文

- [x] Task 13: 回归验证 CRUD 主流程
  - [x] 补充后端接口测试：组织/角色/用户新增、编辑、删除与删除拦截
  - [x] 执行 `npm run build` 验证前端构建
  - [x] 手工验收：三个页面均可完成新增、编辑、删除；角色详情、用户分配角色、用户启停不回退
  - [x] 如项目为前后端分离，构建后重启前端与后端开发服务

- [x] Task 14: 修复原始数据权限种子与 admin 补授权
  - [x] 新增迁移：补齐 `data:import`、`data:read` 权限码（幂等）
  - [x] 新增迁移：为 admin 角色补齐上述权限的 `role_permission` 关系（幂等）
  - [x] 校验当前数据库迁移后 admin 对原始数据页面具备“可读+可导入”最小权限

- [x] Task 15: 稳定 dev-bypass 用户映射策略
  - [x] 调整开发态 token 逻辑：优先命中 admin 角色的启用用户，不再依赖“第一个启用用户”
  - [x] 当不存在可用 admin 用户时，返回明确中文错误并保留排查信息
  - [x] 补充认证层测试，覆盖 dev-bypass 命中与失败分支

- [x] Task 16: 提升原始数据页面错误可见性
  - [x] 原始批次列表初始化失败时展示中文错误提示，不再静默清空列表
  - [x] 上传失败时保留后端错误信息透传（如“无权限访问”），并提示可重试
  - [x] 明确权限不足场景文案，避免用户误判为“点击无反应”

- [x] Task 17: 回归与验收（上传链路）
  - [x] 补充后端测试：缺少权限返回 403、具备权限可上传成功
  - [x] 运行后端测试与 `npm run build`
  - [x] 重启前后端服务并使用真实 Excel 文件完成一次上传自测
  - [x] 验证上传后批次列表可见、状态可更新、错误提示中文且可理解
...
- Task 10 depends on Task 2, Task 3
- Task 11 depends on Task 2, Task 3, Task 4
