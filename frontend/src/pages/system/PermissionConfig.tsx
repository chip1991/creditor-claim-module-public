...
export default function PermissionConfig() {
  ...
  useEffect(() => {
    const fetchConfig = async () => {
      setLoading(true);
      try {
        const res = await api.get('/system/permission/get');
  ...
  const handleSave = async () => {
    setSaving(true);
    try {
      await api.post('/system/permission/save', { value });
    } catch {
    } finally {
      setSaving(false);
    }
  ...
  return (
    <div className="flex flex-col gap-6">
      <div>
        <h1 className="text-2xl font-semibold text-neutral-900">部门与权限配置</h1>
        <p className="text-sm text-neutral-500 mt-1">维护部门、角色、菜单与数据范围权限</p>
      </div>
      ...
      <div className="bg-white border border-neutral-200 rounded-lg shadow-sm p-6">
        ...
        {loading ? (
          ...
          <textarea
            ...
            placeholder="建议使用 JSON/YAML 或按约定格式维护权限配置..."
          />
        )}
