...
export default function RulesConfig() {
  ...
  useEffect(() => {
    const fetchConfig = async () => {
      setLoading(true);
      try {
        const res = await api.get('/system/rules/get');
  ...
  const handleSave = async () => {
    setSaving(true);
    try {
      await api.post('/system/rules/save', { value });
    } catch {
    } finally {
      setSaving(false);
    }
  ...
  return (
    <div className="flex flex-col gap-6">
      <div>
        <h1 className="text-2xl font-semibold text-neutral-900">智能体规则配置</h1>
        <p className="text-sm text-neutral-500 mt-1">配置工单生成、流转、超时预警、满意度校验等规则</p>
      </div>
      ...
      <div className="bg-white border border-neutral-200 rounded-lg shadow-sm p-6">
        ...
        {loading ? (
          ...
          <textarea
            ...
            placeholder="建议使用 JSON/YAML 或按约定格式维护智能体规则..."
          />
        )}
